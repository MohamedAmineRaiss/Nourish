import { Locale } from '@/types';

const en: Record<string, string> = {
  'app.name': 'Nourish',
  'app.tagline': 'Plan meals, meet your nutrition goals',
  'greeting.morning': 'Good morning',
  'greeting.afternoon': 'Good afternoon',
  'greeting.evening': 'Good evening',

  'nav.home': 'Home', 'nav.plan': 'Plan', 'nav.track': 'Track',
  'nav.settings': 'Settings', 'nav.stock': 'Stock', 'nav.suggest': 'Suggest',

  'meal.breakfast': 'Breakfast', 'meal.lunch': 'Lunch', 'meal.dinner': 'Dinner', 'meal.snack': 'Snack',

  'common.save': 'Save', 'common.cancel': 'Cancel', 'common.close': 'Close',
  'common.edit': 'Edit', 'common.delete': 'Delete', 'common.apply': 'Apply',

  // Nutrition filters (replaces old diet.* keys — kept as aliases for safety)
  'filter.highProtein': 'High-protein',
  'filter.highProteinDesc': '20g+ protein per 100g',
  'filter.lowCarb': 'Low-carb / keto',
  'filter.lowCarbDesc': '≤10g carbs per 100g',
  'filter.highIron': 'High-iron',
  'filter.highIronDesc': '3mg+ iron per 100g',
  'filter.lowCalorie': 'Low-calorie',
  'filter.lowCalorieDesc': '≤100 kcal per 100g',
  'filter.highFiber': 'High-fiber',
  'filter.highFiberDesc': '5g+ fiber per 100g',

  // Activity & goal
  'activity.sedentary': 'Sedentary',
  'activity.sedentaryDesc': 'Little or no exercise',
  'activity.light': 'Lightly active',
  'activity.lightDesc': 'Light exercise 1-3×/week',
  'activity.moderate': 'Moderately active',
  'activity.moderateDesc': 'Moderate exercise 3-5×/week',
  'activity.active': 'Active',
  'activity.activeDesc': 'Hard exercise 6-7×/week',
  'activity.very_active': 'Very active',
  'activity.very_activeDesc': 'Hard exercise + physical job',

  'goal.lose': 'Lose weight',
  'goal.loseDesc': '500 kcal deficit',
  'goal.maintain': 'Maintain',
  'goal.maintainDesc': 'Stay at current weight',
  'goal.gain': 'Gain muscle',
  'goal.gainDesc': '300 kcal surplus',

  // Body metrics
  'body.title': 'Personal Info & Macro Calculator',
  'body.subtitle': 'Get macro targets based on your body and goals',
  'body.weight': 'Weight (kg)',
  'body.height': 'Height (cm)',
  'body.age': 'Age',
  'body.sex': 'Sex',
  'body.male': 'Male',
  'body.female': 'Female',
  'body.activity': 'Activity level',
  'body.goal': 'Goal',
  'body.calculated': 'Calculated Targets',
  'body.apply': 'Apply Calculated Targets',
  'body.incomplete': 'Fill in all fields above to see calculated targets',
  'body.bmr': 'Base metabolism',
  'body.tdee': 'Daily expenditure',

  // Update notice
  'update.title': "What's new in v2.2",
  'update.intro': 'A few big improvements since you last used Nourish:',
  'update.foodsTitle': '386 Moroccan foods',
  'update.foodsDesc': 'New food database with whole foods, verified macros, and Moroccan dialect synonyms (kebda, klawi, letchine, hamed…).',
  'update.filtersTitle': 'Smarter filters',
  'update.filtersDesc': 'Vegan/gluten-free chips replaced with high-protein, low-carb (keto), high-iron, low-calorie and high-fiber — computed from real macros, always accurate.',
  'update.calculatorTitle': 'Macro calculator',
  'update.calculatorDesc': 'In Settings, enter your weight, height, age and goal to compute personalized daily targets using the Mifflin-St Jeor formula.',
  'update.gotIt': 'Got it',

  // Onboarding (unchanged)
  'onboarding.skip': 'Skip', 'onboarding.next': 'Next', 'onboarding.start': 'Get Started',
  'onboarding.welcome.title': 'Welcome to Nourish',
  'onboarding.welcome.desc': 'Your personal nutrition planner. Track meals, get smart portion suggestions, and close your daily nutrient rings.',
  'onboarding.plan.title': 'Plan Your Meals',
  'onboarding.plan.desc': 'Search ingredients, pick a meal type, and get quantity suggestions tailored to your remaining needs.',
  'onboarding.stock.title': 'Track Your Stock',
  'onboarding.stock.desc': 'Add groceries to the pantry. Logging a meal auto-deducts stock. Alerts when items run low.',
  'onboarding.suggest.title': 'AI Meal Ideas',
  'onboarding.suggest.desc': 'Get meal suggestions from your stock or any ingredients — powered by AI, in your language.',

  'error.network': 'Network error. Check your connection and try again.',
  'error.timeout': 'Request timed out. Please try again.',
  'error.loadFailed': 'Failed to load. Tap to retry.',
  'error.retry': 'Try Again',

  'dashboard.keyNutrients': 'Key Nutrients Today',
  'dashboard.stillNeeded': 'Still Needed Today',
  'dashboard.stillNeededDesc': 'These nutrients are below 70% of your daily target',
  'dashboard.todaysMeals': "Today's Meals",
  'dashboard.planNext': 'Plan Next Meal',
  'dashboard.noMeals': 'No meals logged yet. Plan your first meal!',
  'dashboard.weeklyReport': 'Weekly Report',

  'plan.title': 'Plan a Meal',
  'plan.subtitle': 'Search and add the ingredients you plan to cook',
  'plan.searchPlaceholder': 'Search ingredients (try Arabic, French, or dialect)',
  'plan.selected': 'Selected Ingredients',
  'plan.generate': 'Generate Quantity Suggestions',
  'plan.searching': 'Searching...',
  'plan.noResults': 'No ingredients found. Try a different search or add a custom ingredient.',
  'plan.mealType': 'What meal is this?',
  'plan.filter': 'Nutrition filters',
  'plan.clearFilters': 'Clear filters',
  'plan.addCustom': 'Add custom ingredient',

  'suggestions.title': 'Suggested Combinations', 'suggestions.subtitle': 'Choose the option that feels right',
  'suggestions.option': 'Option', 'suggestions.score': 'Score',
  'suggestions.choose': 'Choose This Option', 'suggestions.back': 'Back to Ingredients',
  'suggestions.strongIn': 'High in', 'suggestions.helpsWith': 'Helps with', 'suggestions.balances': 'Balances',

  'planned.title': 'Your Planned Meal', 'planned.ingredients': 'Ingredients',
  'planned.editHint': 'Tap any amount to adjust it',
  'planned.provides': 'This Meal Provides', 'planned.addToDay': 'Add This Meal to Today',
  'planned.backSuggestions': 'Back to Suggestions',
  'planned.totalWeight': 'Total weight', 'planned.reset': 'Reset to suggested',

  'tracker.title': 'Daily Tracker',
  'tracker.today': 'Today', 'tracker.week': 'Last 7 days', 'tracker.month': 'Last 30 days',
  'tracker.priority': 'Priority Nutrients', 'tracker.all': 'All Nutrients',
  'tracker.trends': 'Trends', 'tracker.mealsLogged': 'Meals Logged',
  'tracker.resetDay': 'Reset Day', 'tracker.noMeals': 'No meals logged today.',

  'trends.avg': 'avg', 'trends.target': 'target',
  'trends.overDays': 'over {n} days', 'trends.empty': 'No data to show yet',

  'edit.title': 'Edit Meal',
  'edit.confirmDelete': 'Are you sure you want to delete this meal?',
  'edit.deleteConfirm': 'Yes, delete',

  'custom.title': 'Add Custom Ingredient',
  'custom.subtitle': 'Create your own ingredient with nutrient values per 100g',
  'custom.names': 'Name (in your languages)',
  'custom.labelEnPh': 'e.g. Grandma\'s sauce',
  'custom.labelFrPh': 'ex: Sauce de grand-mère',
  'custom.labelArPh': 'مثال: صلصة الجدة',
  'custom.category': 'Category',
  'custom.nutrientsPer100g': 'Nutrients (per 100g)',
  'custom.nutrientsHint': 'Fill in what you know — leave 0 for unknown values',
  'custom.showAdvanced': 'Show advanced nutrients',
  'custom.hideAdvanced': 'Hide advanced nutrients',
  'custom.needLabel': 'Please enter a name in at least one language',

  'stock.title': 'Grocery Stock', 'stock.subtitle': 'Track your kitchen inventory',
  'stock.searchAdd': 'Search to add ingredients...',
  'stock.current': 'Current Stock',
  'stock.empty': 'No items in stock. Search above to add ingredients.',
  'stock.add': 'Add', 'stock.left': 'left', 'stock.lowAlert': 'Low Stock Warning',

  'suggest.title': 'Meal Ideas', 'suggest.subtitle': 'Get AI-powered meal suggestions',
  'suggest.fromStock': 'From my stock', 'suggest.fromCustom': 'Custom ingredients',
  'suggest.generate': 'Get Suggestions', 'suggest.generating': 'Thinking...',
  'suggest.noResults': 'No suggestions available. Try different ingredients.',
  'suggest.ingredientsPlaceholder': 'Enter ingredients, one per line...',
  'suggest.respectingFilters': 'Prioritizing',

  'weekly.title': 'Weekly Report', 'weekly.loading': 'Analyzing your week...',
  'weekly.noData': 'No meals tracked this week yet.',
  'weekly.daysTracked': 'days tracked', 'weekly.strengths': 'Strengths',
  'weekly.gaps': 'Areas to Improve', 'weekly.suggestions': 'Suggestions',
  'weekly.dailyAvg': 'Daily Averages',

  'settings.title': 'Settings', 'settings.profile': 'Profile', 'settings.yourName': 'Your Name',
  'settings.language': 'Language', 'settings.targets': 'Daily Nutrient Targets',
  'settings.targetsNote': 'Consult your healthcare provider for personalized targets',
  'settings.save': 'Save Settings',
  'settings.healthNote': 'This app supports meal planning and nutrient tracking only. It does not diagnose or treat any condition.',
  'settings.resetOnboarding': 'Show Tutorial Again',
  'settings.filters': 'Preferred Nutrition Filters',
  'settings.filtersHint': 'Prioritize these qualities in AI meal suggestions',

  'nutrient.iron': 'Iron', 'nutrient.protein': 'Protein', 'nutrient.iodine': 'Iodine',
  'nutrient.selenium': 'Selenium', 'nutrient.vitaminB12': 'Vitamin B12', 'nutrient.folate': 'Folate',
  'nutrient.vitaminC': 'Vitamin C', 'nutrient.zinc': 'Zinc', 'nutrient.fiber': 'Fiber',
  'nutrient.calories': 'Calories', 'nutrient.carbs': 'Carbs', 'nutrient.fat': 'Fat',

  'toast.mealAdded': 'Meal added! ✅', 'toast.dayReset': 'Day reset! 🌅',
  'toast.settingsSaved': 'Settings saved! 🎯', 'toast.saving': 'Saving...',
  'toast.error': 'Something went wrong. Please try again.',
  'toast.stockLow': 'is running low! Time to restock.',
  'toast.mealUpdated': 'Meal updated ✅', 'toast.mealDeleted': 'Meal deleted',
  'toast.customAdded': 'Custom ingredient created ✨',
  'toast.targetsApplied': 'Calculated targets applied 🎯',
};

const ar: Record<string, string> = {
  'app.name': 'تغذية',
  'app.tagline': 'خططي لوجباتك وحققي أهدافك الغذائية',
  'greeting.morning': 'صباح الخير', 'greeting.afternoon': 'مساء الخير', 'greeting.evening': 'مساء الخير',

  'nav.home': 'الرئيسية', 'nav.plan': 'تخطيط', 'nav.track': 'متابعة',
  'nav.settings': 'إعدادات', 'nav.stock': 'المخزون', 'nav.suggest': 'اقتراحات',

  'meal.breakfast': 'فطور', 'meal.lunch': 'غداء', 'meal.dinner': 'عشاء', 'meal.snack': 'وجبة خفيفة',

  'common.save': 'حفظ', 'common.cancel': 'إلغاء', 'common.close': 'إغلاق',
  'common.edit': 'تعديل', 'common.delete': 'حذف', 'common.apply': 'تطبيق',

  'filter.highProtein': 'غني بالبروتين',
  'filter.highProteinDesc': '20غ+ بروتين لكل 100غ',
  'filter.lowCarb': 'قليل الكربوهيدرات / كيتو',
  'filter.lowCarbDesc': '≤10غ كربوهيدرات لكل 100غ',
  'filter.highIron': 'غني بالحديد',
  'filter.highIronDesc': '3ملغ+ حديد لكل 100غ',
  'filter.lowCalorie': 'قليل السعرات',
  'filter.lowCalorieDesc': '≤100 سعرة لكل 100غ',
  'filter.highFiber': 'غني بالألياف',
  'filter.highFiberDesc': '5غ+ ألياف لكل 100غ',

  'activity.sedentary': 'قليل الحركة',
  'activity.sedentaryDesc': 'تمرين قليل أو لا يوجد',
  'activity.light': 'نشاط خفيف',
  'activity.lightDesc': 'تمرين خفيف 1-3 مرات/أسبوع',
  'activity.moderate': 'نشاط معتدل',
  'activity.moderateDesc': 'تمرين معتدل 3-5 مرات/أسبوع',
  'activity.active': 'نشيط',
  'activity.activeDesc': 'تمرين قوي 6-7 مرات/أسبوع',
  'activity.very_active': 'نشيط جداً',
  'activity.very_activeDesc': 'تمرين قوي + عمل بدني',

  'goal.lose': 'خسارة وزن',
  'goal.loseDesc': 'نقص 500 سعرة',
  'goal.maintain': 'الحفاظ على الوزن',
  'goal.maintainDesc': 'البقاء على الوزن الحالي',
  'goal.gain': 'زيادة عضلات',
  'goal.gainDesc': 'فائض 300 سعرة',

  'body.title': 'المعلومات الشخصية وحاسبة الماكروز',
  'body.subtitle': 'احصلي على أهداف مخصصة حسب جسمك وأهدافك',
  'body.weight': 'الوزن (كغ)',
  'body.height': 'الطول (سم)',
  'body.age': 'العمر',
  'body.sex': 'الجنس',
  'body.male': 'ذكر',
  'body.female': 'أنثى',
  'body.activity': 'مستوى النشاط',
  'body.goal': 'الهدف',
  'body.calculated': 'الأهداف المحسوبة',
  'body.apply': 'تطبيق الأهداف المحسوبة',
  'body.incomplete': 'املئي جميع الحقول أعلاه لرؤية الأهداف المحسوبة',
  'body.bmr': 'الاستقلاب الأساسي',
  'body.tdee': 'الاستهلاك اليومي',

  'update.title': 'الجديد في الإصدار 2.2',
  'update.intro': 'تحسينات مهمة منذ آخر استخدام لتغذية:',
  'update.foodsTitle': '386 طعام مغربي',
  'update.foodsDesc': 'قاعدة بيانات جديدة بأطعمة كاملة وقيم غذائية موثقة ومرادفات باللهجة المغربية (كبدة، كلاوي، لتشينة، حامض...).',
  'update.filtersTitle': 'فلاتر أذكى',
  'update.filtersDesc': 'استبدال فلاتر نباتي/بدون غلوتين بفلاتر: غني بالبروتين، قليل الكربوهيدرات (كيتو)، غني بالحديد، قليل السعرات، وغني بالألياف — محسوبة من القيم الفعلية.',
  'update.calculatorTitle': 'حاسبة الماكروز',
  'update.calculatorDesc': 'في الإعدادات، أدخلي وزنك وطولك وعمرك وهدفك لحساب أهداف يومية مخصصة باستخدام صيغة Mifflin-St Jeor.',
  'update.gotIt': 'فهمت',

  'onboarding.skip': 'تخطي', 'onboarding.next': 'التالي', 'onboarding.start': 'ابدأي الآن',
  'onboarding.welcome.title': 'مرحباً بك في تغذية',
  'onboarding.welcome.desc': 'مخططك الغذائي الشخصي. تتبعي الوجبات، واحصلي على اقتراحات ذكية للكميات.',
  'onboarding.plan.title': 'خططي لوجباتك',
  'onboarding.plan.desc': 'ابحثي عن المكونات، اختاري نوع الوجبة، واحصلي على اقتراحات كميات مصممة حسب احتياجاتك.',
  'onboarding.stock.title': 'تتبعي مخزونك',
  'onboarding.stock.desc': 'أضيفي مشترياتك للمخزن. تسجيل وجبة يخصم المخزون تلقائياً. تنبيهات عند انخفاض الكميات.',
  'onboarding.suggest.title': 'أفكار وجبات بالذكاء الاصطناعي',
  'onboarding.suggest.desc': 'احصلي على اقتراحات من مخزونك أو أي مكونات — بلغتك.',

  'error.network': 'خطأ في الشبكة. تحققي من الاتصال وحاولي مجدداً.',
  'error.timeout': 'انتهت المهلة. يرجى المحاولة مرة أخرى.',
  'error.loadFailed': 'فشل التحميل. اضغطي لإعادة المحاولة.',
  'error.retry': 'أعيدي المحاولة',

  'dashboard.keyNutrients': 'العناصر الغذائية الرئيسية اليوم',
  'dashboard.stillNeeded': 'ما زلت تحتاجين اليوم',
  'dashboard.stillNeededDesc': 'هذه العناصر أقل من 70% من هدفك اليومي',
  'dashboard.todaysMeals': 'وجبات اليوم',
  'dashboard.planNext': 'خططي الوجبة القادمة',
  'dashboard.noMeals': 'لم يتم تسجيل وجبات بعد. خططي لوجبتك الأولى!',
  'dashboard.weeklyReport': 'التقرير الأسبوعي',

  'plan.title': 'خططي لوجبة',
  'plan.subtitle': 'ابحثي وأضيفي المكونات',
  'plan.searchPlaceholder': 'ابحثي عن مكونات (بالعربية أو الفرنسية أو الدارجة)',
  'plan.selected': 'المكونات المختارة',
  'plan.generate': 'إنشاء اقتراحات الكميات',
  'plan.searching': 'جاري البحث...',
  'plan.noResults': 'لم يتم العثور على مكونات.',
  'plan.mealType': 'ما نوع هذه الوجبة؟',
  'plan.filter': 'فلاتر غذائية',
  'plan.clearFilters': 'مسح الفلاتر',
  'plan.addCustom': 'أضيفي مكوناً مخصصاً',

  'suggestions.title': 'التركيبات المقترحة', 'suggestions.subtitle': 'اختاري الخيار المناسب',
  'suggestions.option': 'خيار', 'suggestions.score': 'النتيجة',
  'suggestions.choose': 'اختاري هذا الخيار', 'suggestions.back': 'العودة للمكونات',
  'suggestions.strongIn': 'غني بـ', 'suggestions.helpsWith': 'يساعد في', 'suggestions.balances': 'يوازن',

  'planned.title': 'وجبتك المخططة', 'planned.ingredients': 'المكونات',
  'planned.editHint': 'اضغطي على أي كمية لتعديلها',
  'planned.provides': 'هذه الوجبة توفر', 'planned.addToDay': 'أضيفي هذه الوجبة لليوم',
  'planned.backSuggestions': 'العودة للاقتراحات',
  'planned.totalWeight': 'الوزن الإجمالي', 'planned.reset': 'إعادة للمقترح',

  'tracker.title': 'متابعة يومية',
  'tracker.today': 'اليوم', 'tracker.week': 'آخر 7 أيام', 'tracker.month': 'آخر 30 يوم',
  'tracker.priority': 'العناصر ذات الأولوية', 'tracker.all': 'جميع العناصر',
  'tracker.trends': 'الاتجاهات', 'tracker.mealsLogged': 'الوجبات المسجلة',
  'tracker.resetDay': 'إعادة تعيين اليوم', 'tracker.noMeals': 'لم يتم تسجيل وجبات اليوم.',

  'trends.avg': 'المتوسط', 'trends.target': 'الهدف',
  'trends.overDays': 'خلال {n} أيام', 'trends.empty': 'لا توجد بيانات بعد',

  'edit.title': 'تعديل الوجبة',
  'edit.confirmDelete': 'هل أنت متأكدة من حذف هذه الوجبة؟',
  'edit.deleteConfirm': 'نعم، احذفي',

  'custom.title': 'إضافة مكون مخصص',
  'custom.subtitle': 'أنشئي مكوناً خاصاً بك مع القيم الغذائية لكل 100غ',
  'custom.names': 'الاسم',
  'custom.labelEnPh': 'e.g. Grandma\'s sauce',
  'custom.labelFrPh': 'ex: Sauce de grand-mère',
  'custom.labelArPh': 'مثال: صلصة الجدة',
  'custom.category': 'الفئة',
  'custom.nutrientsPer100g': 'العناصر الغذائية (لكل 100غ)',
  'custom.nutrientsHint': 'املئي ما تعرفينه — اتركي 0 للمجهول',
  'custom.showAdvanced': 'إظهار العناصر المتقدمة',
  'custom.hideAdvanced': 'إخفاء العناصر المتقدمة',
  'custom.needLabel': 'يرجى إدخال اسم بلغة واحدة على الأقل',

  'stock.title': 'مخزون البقالة', 'stock.subtitle': 'تتبعي محتويات مطبخك',
  'stock.searchAdd': 'ابحثي لإضافة مكونات...', 'stock.current': 'المخزون الحالي',
  'stock.empty': 'لا توجد مواد في المخزون.',
  'stock.add': 'أضيفي', 'stock.left': 'متبقي', 'stock.lowAlert': 'تنبيه مخزون منخفض',

  'suggest.title': 'أفكار وجبات', 'suggest.subtitle': 'احصلي على اقتراحات وجبات بالذكاء الاصطناعي',
  'suggest.fromStock': 'من مخزوني', 'suggest.fromCustom': 'مكونات مخصصة',
  'suggest.generate': 'اقتراحات', 'suggest.generating': 'جاري التفكير...',
  'suggest.noResults': 'لا توجد اقتراحات.',
  'suggest.ingredientsPlaceholder': 'أدخلي المكونات، واحد في كل سطر...',
  'suggest.respectingFilters': 'يعطي الأولوية لـ',

  'weekly.title': 'التقرير الأسبوعي', 'weekly.loading': 'جاري تحليل أسبوعك...',
  'weekly.noData': 'لم يتم تتبع وجبات هذا الأسبوع.',
  'weekly.daysTracked': 'أيام تم تتبعها', 'weekly.strengths': 'نقاط القوة',
  'weekly.gaps': 'مجالات للتحسين', 'weekly.suggestions': 'اقتراحات',
  'weekly.dailyAvg': 'المعدلات اليومية',

  'settings.title': 'الإعدادات', 'settings.profile': 'الملف الشخصي', 'settings.yourName': 'اسمك',
  'settings.language': 'اللغة', 'settings.targets': 'الأهداف الغذائية اليومية',
  'settings.targetsNote': 'استشيري مقدم الرعاية الصحية للحصول على أهداف مخصصة',
  'settings.save': 'حفظ الإعدادات',
  'settings.healthNote': 'هذا التطبيق يدعم تخطيط الوجبات وتتبع التغذية فقط.',
  'settings.resetOnboarding': 'عرض الشرح مجدداً',
  'settings.filters': 'الفلاتر الغذائية المفضلة',
  'settings.filtersHint': 'إعطاء الأولوية لهذه الخصائص في اقتراحات الذكاء الاصطناعي',

  'nutrient.iron': 'حديد', 'nutrient.protein': 'بروتين', 'nutrient.iodine': 'يود',
  'nutrient.selenium': 'سيلينيوم', 'nutrient.vitaminB12': 'فيتامين ب12', 'nutrient.folate': 'حمض الفوليك',
  'nutrient.vitaminC': 'فيتامين سي', 'nutrient.zinc': 'زنك', 'nutrient.fiber': 'ألياف',
  'nutrient.calories': 'سعرات', 'nutrient.carbs': 'كربوهيدرات', 'nutrient.fat': 'دهون',

  'toast.mealAdded': 'تم إضافة الوجبة! ✅', 'toast.dayReset': 'تم إعادة تعيين اليوم! 🌅',
  'toast.settingsSaved': 'تم حفظ الإعدادات! 🎯', 'toast.saving': 'جاري الحفظ...',
  'toast.error': 'حدث خطأ. يرجى المحاولة مرة أخرى.',
  'toast.stockLow': 'على وشك النفاد!',
  'toast.mealUpdated': 'تم تحديث الوجبة ✅', 'toast.mealDeleted': 'تم حذف الوجبة',
  'toast.customAdded': 'تم إنشاء المكون المخصص ✨',
  'toast.targetsApplied': 'تم تطبيق الأهداف المحسوبة 🎯',
};

const fr: Record<string, string> = {
  'app.name': 'Nourish',
  'app.tagline': 'Planifiez vos repas, atteignez vos objectifs nutritionnels',
  'greeting.morning': 'Bonjour', 'greeting.afternoon': 'Bon après-midi', 'greeting.evening': 'Bonsoir',

  'nav.home': 'Accueil', 'nav.plan': 'Planifier', 'nav.track': 'Suivi',
  'nav.settings': 'Réglages', 'nav.stock': 'Stock', 'nav.suggest': 'Idées',

  'meal.breakfast': 'Petit-déj', 'meal.lunch': 'Déjeuner', 'meal.dinner': 'Dîner', 'meal.snack': 'Snack',

  'common.save': 'Enregistrer', 'common.cancel': 'Annuler', 'common.close': 'Fermer',
  'common.edit': 'Modifier', 'common.delete': 'Supprimer', 'common.apply': 'Appliquer',

  'filter.highProtein': 'Riche en protéines',
  'filter.highProteinDesc': '20g+ de protéines pour 100g',
  'filter.lowCarb': 'Faible en glucides / keto',
  'filter.lowCarbDesc': '≤10g de glucides pour 100g',
  'filter.highIron': 'Riche en fer',
  'filter.highIronDesc': '3mg+ de fer pour 100g',
  'filter.lowCalorie': 'Peu calorique',
  'filter.lowCalorieDesc': '≤100 kcal pour 100g',
  'filter.highFiber': 'Riche en fibres',
  'filter.highFiberDesc': '5g+ de fibres pour 100g',

  'activity.sedentary': 'Sédentaire',
  'activity.sedentaryDesc': 'Peu ou pas d\'exercice',
  'activity.light': 'Peu actif',
  'activity.lightDesc': 'Exercice léger 1-3×/sem',
  'activity.moderate': 'Modérément actif',
  'activity.moderateDesc': 'Exercice modéré 3-5×/sem',
  'activity.active': 'Actif',
  'activity.activeDesc': 'Exercice intense 6-7×/sem',
  'activity.very_active': 'Très actif',
  'activity.very_activeDesc': 'Exercice intense + travail physique',

  'goal.lose': 'Perdre du poids',
  'goal.loseDesc': 'Déficit de 500 kcal',
  'goal.maintain': 'Maintenir',
  'goal.maintainDesc': 'Rester au poids actuel',
  'goal.gain': 'Prendre du muscle',
  'goal.gainDesc': 'Surplus de 300 kcal',

  'body.title': 'Infos perso & Calculateur de macros',
  'body.subtitle': 'Obtenez des objectifs adaptés à votre corps',
  'body.weight': 'Poids (kg)',
  'body.height': 'Taille (cm)',
  'body.age': 'Âge',
  'body.sex': 'Sexe',
  'body.male': 'Homme',
  'body.female': 'Femme',
  'body.activity': "Niveau d'activité",
  'body.goal': 'Objectif',
  'body.calculated': 'Objectifs calculés',
  'body.apply': 'Appliquer les objectifs calculés',
  'body.incomplete': 'Remplissez tous les champs pour voir les objectifs',
  'body.bmr': 'Métabolisme de base',
  'body.tdee': 'Dépense quotidienne',

  'update.title': 'Nouveautés v2.2',
  'update.intro': 'Quelques améliorations majeures depuis votre dernière visite :',
  'update.foodsTitle': '386 aliments marocains',
  'update.foodsDesc': 'Nouvelle base avec des aliments bruts, macros vérifiées et synonymes en dialecte (kebda, klawi, letchine…).',
  'update.filtersTitle': 'Filtres plus intelligents',
  'update.filtersDesc': 'Végane/sans-gluten remplacés par : riche en protéines, pauvre en glucides (keto), riche en fer, peu calorique, riche en fibres — calculés à partir des vraies macros.',
  'update.calculatorTitle': 'Calculateur de macros',
  'update.calculatorDesc': 'Dans Réglages, entrez poids, taille, âge et objectif pour obtenir des cibles personnalisées (formule Mifflin-St Jeor).',
  'update.gotIt': 'Compris',

  'onboarding.skip': 'Passer', 'onboarding.next': 'Suivant', 'onboarding.start': 'Commencer',
  'onboarding.welcome.title': 'Bienvenue sur Nourish',
  'onboarding.welcome.desc': 'Votre planificateur nutritionnel. Suivez vos repas et complétez vos anneaux.',
  'onboarding.plan.title': 'Planifiez vos repas',
  'onboarding.plan.desc': 'Cherchez des ingrédients et obtenez des suggestions de portions.',
  'onboarding.stock.title': 'Suivez votre stock',
  'onboarding.stock.desc': 'Ajoutez vos courses. Le stock se déduit automatiquement lors des repas.',
  'onboarding.suggest.title': 'Idées repas par IA',
  'onboarding.suggest.desc': 'Obtenez des suggestions basées sur votre stock ou vos ingrédients.',

  'error.network': 'Erreur réseau. Vérifiez votre connexion et réessayez.',
  'error.timeout': 'Délai dépassé. Veuillez réessayer.',
  'error.loadFailed': 'Échec du chargement. Appuyez pour réessayer.',
  'error.retry': 'Réessayer',

  'dashboard.keyNutrients': "Nutriments clés aujourd'hui",
  'dashboard.stillNeeded': "Encore nécessaire aujourd'hui",
  'dashboard.stillNeededDesc': 'Ces nutriments sont en dessous de 70% de votre objectif',
  'dashboard.todaysMeals': "Repas d'aujourd'hui",
  'dashboard.planNext': 'Planifier le prochain repas',
  'dashboard.noMeals': 'Aucun repas enregistré. Planifiez votre premier repas !',
  'dashboard.weeklyReport': 'Rapport hebdomadaire',

  'plan.title': 'Planifier un repas',
  'plan.subtitle': 'Cherchez et ajoutez les ingrédients',
  'plan.searchPlaceholder': 'Chercher (arabe, français, dialecte)',
  'plan.selected': 'Ingrédients sélectionnés',
  'plan.generate': 'Générer des suggestions',
  'plan.searching': 'Recherche en cours...',
  'plan.noResults': 'Aucun ingrédient trouvé.',
  'plan.mealType': 'Quel type de repas ?',
  'plan.filter': 'Filtres nutritionnels',
  'plan.clearFilters': 'Effacer les filtres',
  'plan.addCustom': 'Ajouter un ingrédient personnalisé',

  'suggestions.title': 'Combinaisons suggérées', 'suggestions.subtitle': "Choisissez l'option qui vous convient",
  'suggestions.option': 'Option', 'suggestions.score': 'Score',
  'suggestions.choose': 'Choisir cette option', 'suggestions.back': 'Retour aux ingrédients',
  'suggestions.strongIn': 'Riche en', 'suggestions.helpsWith': 'Aide avec', 'suggestions.balances': 'Équilibre',

  'planned.title': 'Votre repas planifié', 'planned.ingredients': 'Ingrédients',
  'planned.editHint': 'Appuyez sur une quantité pour la modifier',
  'planned.provides': 'Ce repas fournit', 'planned.addToDay': "Ajouter ce repas à aujourd'hui",
  'planned.backSuggestions': 'Retour aux suggestions',
  'planned.totalWeight': 'Poids total', 'planned.reset': 'Remettre la suggestion',

  'tracker.title': 'Suivi quotidien',
  'tracker.today': "Aujourd'hui", 'tracker.week': '7 derniers jours', 'tracker.month': '30 derniers jours',
  'tracker.priority': 'Nutriments prioritaires', 'tracker.all': 'Tous les nutriments',
  'tracker.trends': 'Tendances', 'tracker.mealsLogged': 'Repas enregistrés',
  'tracker.resetDay': 'Réinitialiser la journée', 'tracker.noMeals': "Aucun repas enregistré aujourd'hui.",

  'trends.avg': 'moy', 'trends.target': 'objectif',
  'trends.overDays': 'sur {n} jours', 'trends.empty': 'Pas encore de données',

  'edit.title': 'Modifier le repas',
  'edit.confirmDelete': 'Supprimer ce repas ?',
  'edit.deleteConfirm': 'Oui, supprimer',

  'custom.title': 'Ajouter un ingrédient personnalisé',
  'custom.subtitle': 'Créez votre propre ingrédient (valeurs pour 100g)',
  'custom.names': 'Nom',
  'custom.labelEnPh': 'e.g. Grandma\'s sauce',
  'custom.labelFrPh': 'ex: Sauce de grand-mère',
  'custom.labelArPh': 'مثال: صلصة الجدة',
  'custom.category': 'Catégorie',
  'custom.nutrientsPer100g': 'Nutriments (pour 100g)',
  'custom.nutrientsHint': 'Remplissez ce que vous savez — 0 pour les inconnus',
  'custom.showAdvanced': 'Afficher nutriments avancés',
  'custom.hideAdvanced': 'Masquer nutriments avancés',
  'custom.needLabel': 'Entrez un nom dans au moins une langue',

  'stock.title': 'Stock alimentaire', 'stock.subtitle': 'Suivez votre inventaire',
  'stock.searchAdd': 'Chercher pour ajouter...', 'stock.current': 'Stock actuel',
  'stock.empty': 'Aucun article en stock.',
  'stock.add': 'Ajouter', 'stock.left': 'restant', 'stock.lowAlert': 'Alerte stock faible',

  'suggest.title': 'Idées de repas', 'suggest.subtitle': 'Suggestions par IA',
  'suggest.fromStock': 'Depuis mon stock', 'suggest.fromCustom': 'Ingrédients personnalisés',
  'suggest.generate': 'Obtenir des idées', 'suggest.generating': 'Réflexion...',
  'suggest.noResults': 'Pas de suggestions.',
  'suggest.ingredientsPlaceholder': 'Ingrédients, un par ligne...',
  'suggest.respectingFilters': 'Priorise',

  'weekly.title': 'Rapport hebdomadaire', 'weekly.loading': 'Analyse...',
  'weekly.noData': 'Pas encore de repas suivis cette semaine.',
  'weekly.daysTracked': 'jours suivis', 'weekly.strengths': 'Points forts',
  'weekly.gaps': "Axes d'amélioration", 'weekly.suggestions': 'Suggestions',
  'weekly.dailyAvg': 'Moyennes quotidiennes',

  'settings.title': 'Réglages', 'settings.profile': 'Profil', 'settings.yourName': 'Votre nom',
  'settings.language': 'Langue', 'settings.targets': 'Objectifs nutritionnels quotidiens',
  'settings.targetsNote': 'Consultez un professionnel de santé',
  'settings.save': 'Enregistrer', 'settings.healthNote': 'Cette application aide à la planification des repas uniquement.',
  'settings.resetOnboarding': 'Revoir le tutoriel',
  'settings.filters': 'Filtres nutritionnels préférés',
  'settings.filtersHint': 'Priorisés dans les suggestions IA',

  'nutrient.iron': 'Fer', 'nutrient.protein': 'Protéines', 'nutrient.iodine': 'Iode',
  'nutrient.selenium': 'Sélénium', 'nutrient.vitaminB12': 'Vitamine B12', 'nutrient.folate': 'Folate',
  'nutrient.vitaminC': 'Vitamine C', 'nutrient.zinc': 'Zinc', 'nutrient.fiber': 'Fibres',
  'nutrient.calories': 'Calories', 'nutrient.carbs': 'Glucides', 'nutrient.fat': 'Lipides',

  'toast.mealAdded': 'Repas ajouté ! ✅', 'toast.dayReset': 'Journée réinitialisée ! 🌅',
  'toast.settingsSaved': 'Réglages enregistrés ! 🎯', 'toast.saving': 'Enregistrement...',
  'toast.error': "Quelque chose s'est mal passé.",
  'toast.stockLow': 'bientôt épuisé !',
  'toast.mealUpdated': 'Repas mis à jour ✅', 'toast.mealDeleted': 'Repas supprimé',
  'toast.customAdded': 'Ingrédient personnalisé créé ✨',
  'toast.targetsApplied': 'Objectifs calculés appliqués 🎯',
};

const dictionaries: Record<Locale, Record<string, string>> = { en, ar, fr };

export function t(key: string, locale: Locale): string {
  return dictionaries[locale]?.[key] || dictionaries.en[key] || key;
}

export function isRTL(locale: Locale): boolean {
  return locale === 'ar';
}

export const LOCALE_LABELS: Record<Locale, string> = {
  en: 'English', ar: 'العربية', fr: 'Français',
};
